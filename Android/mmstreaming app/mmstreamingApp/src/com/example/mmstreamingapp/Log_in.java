...
public class Log_in extends Activity {
		...
		if(msg.equalsIgnoreCase("login"))
		{
			...
			login.setOnTouchListener(new OnTouchListener() {
				...
				public boolean onTouch(View arg0, MotionEvent arg1) 
				{
					...
					{
						arg0.setBackgroundResource(R.drawable.login);
						String user = userName.getText().toString();
						String password = Password.getText().toString();
						new login(user, password, Log_in.this).execute();
					}
					return true;
				}
			});
			forgot.setOnTouchListener(new OnTouchListener() {
	...
	public class login extends AsyncTask<Void, Void, Void>
	{
		...
		public login(String user, String pass, Activity a)
		{
			this.u = user;
			this.p = pass;
			this.a = a;
		...
		protected void onPostExecute(Void result) 
		{
			...
			if(canstants.User_Name.equalsIgnoreCase("Log in failed"))
			{
			...
			{
				Toast.makeText(getApplicationContext(), canstants.User_Name, Toast.LENGTH_SHORT).show();
				if(src.equalsIgnoreCase("abstract"))
				{
